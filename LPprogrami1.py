...
def izracunaj_rute(broj_klijenata, routes, vehicles, osobine, prebacivanje, rutabroj):
    ...
    routing_model += 1000*LpAffineExpression((x[key],routes[dictOfroutes[key]][1]) for key in keys)
...
def printaj_rute(rute_iz_res, osobine, instanca, tip, rute_vozila, broj_ruta, ws3, objective, status, benchmark_broj_ruta,benchmark_objective,ukupnopozineg, pozitivnih,negativnih, kapacitet, taboo, ukupno_mogucih_ruta, calc_time):
    ukupno_ruta=0
    for v in broj_ruta:
        ukupno_ruta=ukupno_ruta+broj_ruta[v];ukupno_trajanje_ruta=0;ukupno_trajanje_ui=0;ukupno_poz_pll=0;ukupno_neg_pll=0; ukupno_klijenata=0;ukupno_trajanje_voznje=0
        #print "Broj ruta " + str (v) +" je: "+ str (broj_ruta[v])
        for r in rute_vozila[v]:
            ...
            ukupno_trajanje_ruta=ukupno_trajanje_ruta+rute_iz_res[r][10]
            ukupno_trajanje_ui=ukupno_trajanje_ui+abs(rute_iz_res[r][8])
            ukupno_poz_pll=ukupno_poz_pll+rute_iz_res[r][5]
            ukupno_neg_pll=ukupno_neg_pll+rute_iz_res[r][6]
            ukupno_klijenata=ukupno_klijenata+rute_iz_res[r][9]
            ukupno_trajanje_voznje=ukupno_trajanje_voznje+rute_iz_res[r][11]
    ...
    procenat=str(round((100*(objective-float(benchmark_objective))/float(benchmark_objective)),2))
    print("Razlika: " + str (objective-float(benchmark_objective)) + " / " + procenat + "%")
    
    ws3.append([instanca, tip, objective, benchmark_objective, razlika_objective, procenat, ukupno_ruta, razlika_br_ruta, ukupno_klijenata, status, ukupno_mogucih_ruta, ukupno_poz_pll, ukupno_neg_pll, kapacitet, ukupno_trajanje_ui, ukupno_trajanje_voznje, ukupno_trajanje_ruta, calc_time])
